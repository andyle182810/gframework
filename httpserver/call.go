...
func ExecuteStandardized[REQ any, RES any](
	...
	log := zerolog.Ctx(echoCtx.Request().Context()).With().Str("handler", handlerName).Logger()

	internalResponse, delegateError := delegate(log, echoCtx, request)
	...
	requestID, _ := echoCtx.Get(middleware.ContextKeyRequestID).(string)
